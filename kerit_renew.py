...
基于朋友的 VPS 成功运行流程重写
流程：Discord登录 → 首页面 → Cloudflare过盾 → Discord OAuth → 续期页 → Renew → Sponsor → 停留18秒 → 关闭Sponsor → 再次点击Renew Server → Complete Renewal
"""
import time
import os
import json
...
class KeritCloudRenewal:
    ...
    def click_sponsor_and_complete_renew(self, sb):
        ...
        # ================= 新增：再次点击 Renew Server 唤出弹窗 =================
        self.log("🔄 再次点击 Renew Server 唤出 Complete Renewal 弹窗...")
        try:
            sb.execute_script("""
