...
class KeritCloudRenewal:
    ...
    def _locate_complete_renewal_all_windows(self, sb, main_handle):
        """全窗口查找 Complete Renewal：先主窗口（主文档+iframe），找不到再遍历其他窗口。
        返回 (window_handle, frame_idx_or_None, info)；找不到返回 (None, None, None)。
        注意：无论在哪找到，返回前都切回主窗口，避免后续取 URL / 状态出错。"""
        try:
            # 先查主窗口
            try:
            ...
            if info and info.get('count', 0) > 0:
                return (main_handle, frame_idx, info)
            # 主窗口没有 → 遍历所有窗口（含 sponsor 窗口）
            for h in list(sb.driver.window_handles):
                ...
                frame_idx, info = self._locate_complete_renewal(sb)
                if info and info.get('count', 0) > 0:
                    self.log(f"   🧩 在 sponsor 窗口找到 Complete Renewal")
                    try:
                        sb.driver.switch_to.window(main_handle)
                    except Exception:
                        pass
                    return (h, frame_idx, info)
            # 都没找到 → 切回主窗口
            try:
                sb.driver.switch_to.window(main_handle)
            except Exception:
            ...
            self.log(f"   ⚠️ 全窗口查找异常: {str(e)[:50]}")
            return (None, None, None)

    def _click_complete_renewal_in_window(self, sb, win_handle, frame_idx):
        """切到指定窗口后点击 Complete Renewal。frame_idx=None 表示主文档。"""
        try:
            if win_handle is not None:
                try:
                    sb.driver.switch_to.window(win_handle)
                except Exception as e:
                    self.log(f"   ⚠️ 切换窗口失败: {str(e)[:50]}")
            if frame_idx is not None:
                sb.driver.switch_to.frame(frame_idx)
            result = sb.execute_script("""
                (function(){
    ...
    def click_sponsor_and_complete_renew(self, sb):
        ...
        流程（完全模拟手动操作，单次线性，任一检查不过立即终止本次续期）：
          读天数 → 点 Sponsor → 浏览器健康检查 → 确认 Sponsor 窗口打开
          → 【不关闭 Sponsor 窗口】切回主窗口续期页
          → 定位 Complete Renewal（主文档+iframe）→ 点击前确认 → 点一次 → 验证天数
        注意：所有检查只执行一次，任何一项没过就【关闭本次续期】返回失败，
              绝不重试、绝不继续，因为机会只有一次，状态不对重试只会浪费。
        关键：手动流程是「点 sponsor 后直接在主窗口点 Complete Renewal，从不关 sponsor 窗口」，
              关闭 sponsor 窗口会导致按钮/弹窗状态丢失（主文档+iframe 均找不到）。
        ...
        # ===== 检查③：确认 Sponsor 窗口已打开，并记录主窗口 handle =====
        # 手动操作真相：点 Sponsor 后【绝不关窗口】，主窗口弹窗里 Complete Renewal 直接变亮可点
        # 之前版本「关闭 Sponsor 窗口」会把按钮/弹窗状态弄丢（主文档+iframe 都找不到），故彻底废弃
        self.log("⏳ 检查③：确认 Sponsor 窗口已打开...")
        main_handle = None
        sponsor_ok = False
                    ...
                    for h in list(sb.driver.window_handles):
                        try:
                            ...
                            if "billing.kerit.cloud" not in sb.driver.current_url:
                                sponsor_ok = True   # sponsor 窗口存在
                            else:
                                main_handle = h     # 记录主窗口（续期页）
                        except Exception:
                            continue
                    if sponsor_ok and main_handle:
                        break
            except Exception as e:
                self.log(f"   ⚠️ 等待 Sponsor 窗口重试: {str(e)[:50]}")
            time.sleep(2)
        if not sponsor_ok or not main_handle:
            self.log("❌ 检查③失败（Sponsor 窗口未打开或找不到主窗口），关闭本次续期")
            return (False, days_before, -1)
        self.log("✅ 检查③通过：Sponsor 窗口已打开")

        # ===== 检查④：不关闭 Sponsor 窗口，切回主窗口（续期页）=====
        # 关键修复：关闭 sponsor 窗口会导致主窗口弹窗/Complete Renewal 按钮消失，
        # 完全模拟手动流程 = 留着 sponsor 窗口，直接回主窗口点按钮
        self.log("🔎 检查④：切回主窗口（不关闭 Sponsor 窗口）...")
        back_ok = False
        try:
            sb.driver.switch_to.window(main_handle)
            ...
            self.log(f"   ⚠️ 切换主窗口异常: {str(e)[:60]}")
        if not back_ok:
            self.log("❌ 检查④失败（未找到续期页窗口），关闭本次续期")
            return (False, days_before, -1)
        self.log(f"✅ 检查④通过：已切回续期页 {sb.driver.current_url}")
        ...
        # ===== 检查⑥：定位 Complete Renewal（主文档 + iframe；主窗口找不到再全窗口找）=====
        # 手动流程真相：点 sponsor 后 Complete Renewal 直接在主窗口弹窗里亮起，无需关任何窗口
        # 但按钮可能在主窗口弹窗，也可能在 sponsor 窗口，故：主窗口优先，找不到遍历所有窗口
        self.log("⏳ 检查⑥：定位 Complete Renewal 按钮...")
        btn_win = None       # 按钮所在窗口 handle
        btn_loc = None       # None=主文档, int=iframe 索引
        btn_info = None
        for attempt in range(5):  # 5次 × 1.5s ≈ 8s（检测到立即 break）
            win_h, frame_idx, info = self._locate_complete_renewal_all_windows(sb, main_handle)
            if info and info.get('count', 0) > 0:
                btn_win = win_h
                btn_loc = frame_idx
                btn_info = info
                where = "主文档" if frame_idx is None else f"iframe[{frame_idx}]"
                self.log(f"   ✅ 检查⑥通过：找到 Complete Renewal（窗口={'主' if win_h == main_handle else 'Sponsor'} | {where}，尝试 {attempt+1}/5）")
                break
        ...
        # 按钮在主窗口 = 必须确认主窗口在续期页；按钮在 sponsor 窗口 = 不要求 billing 域（sponsor 域正常）
        try:
            if btn_win == main_handle:
                final_url = sb.driver.current_url
                url_ok = "billing.kerit.cloud" in (final_url or "")
                self.log(f"🔎 检查⑦：URL={'OK' if url_ok else 'FAIL'} | 按钮位置=主窗口{'主文档' if btn_loc is None else f'iframe[{btn_loc}]'}")
                if not url_ok:
                    self.log(f"❌ 检查⑦失败（主窗口不在续期页: {final_url}），关闭本次续期")
                    return (False, days_before, -1)
            else:
                self.log("🔎 检查⑦：按钮在 Sponsor 窗口，跳过 URL 校验")
        except Exception as e:
            self.log(f"❌ 检查⑦异常: {str(e)[:60]}，关闭本次续期")
            return (False, days_before, -1)
        self.log("✅ 检查⑦通过：全部就绪，开始点击 Complete Renewal")

        # ===== 点击 Complete Renewal（只点一次，绝不重复；先切到按钮所在窗口，再按 frame 点击）=====
        try:
            self.log("🖱️ 点击 Complete Renewal...")
            clicked = self._click_complete_renewal_in_window(sb, btn_win, btn_loc)
            if clicked == 'clicked':
                where = "主文档" if btn_loc is None else f"iframe[{btn_loc}]"
                self.log(f"✅ 已点击 Complete Renewal（窗口={'主' if btn_win == main_handle else 'Sponsor'} | {where}）")
            else:
        ...
        # 点击后切回主窗口（无论按钮在哪个窗口，验证都回主窗口续期页读天数）
        try:
            sb.driver.switch_to.window(main_handle)
        except Exception:
            pass
